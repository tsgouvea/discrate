...
assert(TaskParameters.GUI.FracForced >= 0 & TaskParameters.GUI.FracForced <= 1)
...
sma = AddState(sma, 'Name', 'state_0',...
    ...
    'OutputActions', {});
if rand > TaskParameters.GUI.FracForced
    sma = AddState(sma, 'Name', 'wait_Cin',...
        'Timer', 0,...
        'StateChangeConditions', {CenterPortIn, 'free_wait_Sin'},...
        'OutputActions', {strcat('PWM',num2str(CenterPort)),255});
else
    if rand > .5
        sma = AddState(sma, 'Name', 'wait_Cin',...
            'Timer', 0,...
            'StateChangeConditions', {CenterPortIn, 'forc_wait_Lin'},...
            'OutputActions', {strcat('PWM',num2str(CenterPort)),255});
    else
        sma = AddState(sma, 'Name', 'wait_Cin',...
            'Timer', 0,...
            'StateChangeConditions', {CenterPortIn, 'forc_wait_Rin'},...
            'OutputActions', {strcat('PWM',num2str(CenterPort)),255});
    end
end

sma = AddState(sma, 'Name', 'free_wait_Sin',...
    'Timer', 0,...
...
sma = AddState(sma, 'Name', 'forc_wait_Lin',...
    'Timer', 0,...
    'StateChangeConditions', {LeftPortIn,'PreL'},...
    'OutputActions',{strcat('PWM',num2str(LeftPort)),255});
sma = AddState(sma, 'Name', 'forc_wait_Rin',...
    'Timer', 0,...
    'StateChangeConditions', {RightPortIn,'PreR'},...
